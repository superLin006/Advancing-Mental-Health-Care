...
# 文件夹路径，替换为你保存文件的实际路径
tokenizer = BertTokenizer.from_pretrained(model_path,cache_dir=None)
model = BertModel.from_pretrained(model_path,cache_dir=None)
...
def main():
    ...
    question_template_embedding = get_bert_embedding(question_template)
    ...
    for pair, similarity, manhattan_distance, euclidean_distance in sorted_pairs_by_semantic_similarity[:15]:
        question, answer = pair
       # print(f"Question: {question}, Answer: {answer}, Similarity: {similarity}, Manhattan Distance: {manhattan_distance}, Euclidean Distance: {euclidean_distance}")
        print(f" Similarity: {similarity}, Manhattan Distance: {manhattan_distance}, Euclidean Distance: {euclidean_distance}")
    # 输出均值
