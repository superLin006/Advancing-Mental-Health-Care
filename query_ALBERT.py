...
tokenizer = AlbertTokenizer.from_pretrained(model_path, cache_dir=None)
model = AlbertModel.from_pretrained(model_path, cache_dir=None)
...
def generate_question_template(keywords):
    ...
    return f"[low_weight_str]，患有[high_weight_str]，伴有[low_weight_str]等症状，应如何治疗？用什么药？有什么建议？"
...
def calculate_similarity(pair, question_template_embedding):
    _, answer = pair
    answer_embedding = get_albert_embedding(answer)
...
def main():
    ...
    question_template_embedding = get_albert_embedding(question_template)
