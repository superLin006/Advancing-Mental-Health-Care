...
# 假设这是从文件中读取的数据
data_entries = [
"请你帮我整理以下内容，要求内容逻辑清晰，语言表达流畅，并且符合实际的医疗建议："
...
# 可能标志建议开始的关键词或短语
advice_keywords = ["意见建议：", "建议：", "指导意见：", "指导意见", "建议", "针对", "服用", "口服", "应"]
...
for entry in data_entries:
    ...
    for keyword in advice_keywords:
        if keyword in entry:
            # 根据关键词分割文本
            parts = entry.split(keyword)
            if len(parts) > 1:
                # 提取关键词之后的部分，并去除两端的空白字符
                advice = parts[1].strip()
                # 使用正则表达式去除标点符号和多余的空格
                advice = re.sub(r'\s+', ' ', advice)
                treatment_advices.append(advice)
                break  # 假设每条记录只有一个建议部分

# 打印提取的建议
for advice in treatment_advices:
    print(advice)
