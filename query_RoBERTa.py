...
# 删除模型文件和缓存（可选）
tokenizer = RobertaTokenizer.from_pretrained(model_path, cache_dir=None)
model = RobertaModel.from_pretrained(model_path, cache_dir=None)
...
def calculate_similarity(pair, question_template_embedding):
    _, answer = pair
    answer_embedding = get_roberta_embedding(answer)
...
def main():
    ...
    question_template_embedding = get_roberta_embedding(question_template)
